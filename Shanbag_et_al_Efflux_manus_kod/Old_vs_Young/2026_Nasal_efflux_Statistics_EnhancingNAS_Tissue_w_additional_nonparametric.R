...
directory_name = 'NAS_Tissue_w_additional_nonparametric' # Should change for each experiment type 
dir.create(directory_name)

# Read in experiment data file 
setwd('E://Efflux_Paper//Data//For_Manuscript//Excel_Files//Old_vs_Young')
data <- read.csv(file = 'E://Efflux_Paper//Data//For_Manuscript//Excel_Files//Old_vs_Young//20240530_old_vs_young_NAS_tissue_master.csv', header=TRUE, sep=',')
...
final_cumulative_enhancement_wilcoxon = wilcox.test(Cumulative_Sum~Group, data=data_final_time_step, paired = FALSE, p.adjust.method = "BH")
...
enhancement_volume_wilcoxon = wilcox.test(Num_Voxels~Group, data=data_final_time_step, paired = FALSE, p.adjust.method = "BH")
...
cat("\n\n Final Cumulative Enhancement Wilcoxon W\n", file = filename_txt, append=TRUE, sep = "\n")
captureOutput(final_cumulative_enhancement_wilcoxon, file = filename_txt, append = TRUE)
...
cat("\n\n Enhancement Volume Wilcoxon W\n", file = filename_txt, append=TRUE, sep = "\n")
captureOutput(enhancement_volume_wilcoxon, file = filename_txt, append = TRUE)
close(filename_txt)
...
final_cumulative_enhancement_wilcoxon_normalized = wilcox.test(Cumulative_Sum_Normalized~Group, data=data_final_time_step, paired = FALSE, p.adjust.method = "BH")
...
cat("\n\n Final Cumulative Enhancement Wilcoxon Test\n", file = filename_txt, append=TRUE, sep = "\n")
captureOutput(final_cumulative_enhancement_wilcoxon_normalized, file = filename_txt, append = TRUE)
